...
export class EventGateway {
  ...
  @SubscribeMessage(WsEvent.ADD_USER)
  ...
  emitError(roomId: string, error: Error, locale = Locale.EN_US) {
  ...
  // @Interval(1000)
  emitNewMessage(roomId: string, data: WsResponse) {
    // this.server
    //   .to(ROOM)
    //   .emit(WsEvent.NEW_MESSAGE, { message: new Date().toLocaleString() });
    this.server.to(roomId).emit(WsEvent.NEW_MESSAGE, data);
  }
}
